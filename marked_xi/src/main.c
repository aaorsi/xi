...
//	Input Parameters:
	...
	nXX				= atoi(argv[3]);
	Seed			= atoi(argv[4]);
	strcpy(ParticleData,argv[5]);
	strcpy(RandomData,argv[6]);
	DataFormat		= atoi(argv[7]);
	strcpy(OutDir,argv[8]);
	strcpy(PeriodicFlag,argv[9]);
	XLength			= atof(argv[10]);
	YLength			= atof(argv[11]);
	ZLength			= atof(argv[12]);
	strcpy(RandomFlag,argv[13]);
	RanFrac			= atof(argv[14]);
	RanNRun			= atoi(argv[15]);
	CellSizeX		= atof(argv[16]);
	NNeighbour		= atoi(argv[17]);
	strcpy(DensityFieldFlag,argv[18]);
	MassAss			= atoi(argv[19]);
	FPSizeX			= atof(argv[20]);
	strcpy(LogBinFlag,argv[21]);
	BinSize			= atof(argv[22]);
	RMin			= atof(argv[23]);
	RMax			= atof(argv[24]);
	strcpy(OutDD,argv[25]);
	strcpy(OutDR,argv[26]);
	strcpy(OutRR,argv[27]);
	strcpy(CheckParamFlag,argv[28]);
